...
for i in range(len(height_above_terrain)):
    if(i < 250):
        deltaH[i] = (np.tan(dna)*np.tan(np.radians(terrain_angle_1)))/(1+np.tan(dna)*np.tan(np.radians(terrain_angle_1)))*geomH[i]
        #deltaH[i] = geomH[i] * (np.sin(np.radians(terrain_angle_1)))**2
    if(i > 250 and i < 500):
        deltaH[i] = (np.tan(dna)*np.tan(np.radians(terrain_angle_1)))/(1+np.tan(dna)*np.tan(np.radians(terrain_angle_1)))*geomH[i]
        #deltaH[i] = geomH[i] * (np.sin(np.radians(terrain_angle_1)))**2
    if(i >= 500 and i <= 1750):
        #deltaH[i] = (np.tan(dna)*np.tan(np.radians(terrain_angle_3)))/(1+np.tan(dna)*np.tan(np.radians(terrain_angle_3)))*geomH[i]
        deltaH[i] = geomH[i] * (np.sin(np.radians(terrain_angle_3)))**2
        print(geomH[i])
    if(i > 1750 and i <= 2000):
        deltaH[i] = (np.tan(dna)*np.tan(np.radians(terrain_angle_4)))/(1+np.tan(dna)*np.tan(np.radians(terrain_angle_4)))*geomH[i]
        #deltaH[i] = geomH[i] * (np.sin(np.radians(terrain_angle_4)))**2
    if(i > 220 and i < 280  ):
        deltaH[i] = 0
