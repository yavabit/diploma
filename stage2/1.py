    ...
    for i in range(trajectory_x.shape[0]):
        ...
        for search_step in range(-search_range, search_range + 1):
            ...
            for beam_idx, angle_offset in enumerate([-angle_between_beams_rad / 2, angle_between_beams_rad / 2]):
                ...
                projected_x = x_val + search_step * np.cos(pitch_angle_rad[i] + angle_offset) * np.cos(roll_offset)
                projected_y = y_val + search_step * np.sin(pitch_angle_rad[i] + angle_offset) * np.cos(roll_offset) + search_step * np.sin(roll_offset)
                projected_z = z_val + search_step * np.sin(pitch_angle_rad[i])
